  ...
  useEffect(() => {
    ...
    const loadLatest = async () => {
      const { data, error: loadError } = await supabase
        .from('devices')
        .select('spors_key, ble_device_uuid')
        .eq('id', deviceId)
        .maybeSingle()
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.content}>
        <View style={styles.successWrap}>
          ...
          <Text style={styles.subtitle}>Your secure SPORS key will appear shortly.</Text>
        </View>

        <View style={styles.keyCard}>
          <Text style={styles.keyLabel}>SPORS Key</Text>

          {sporsKey ? (
            <Text style={styles.keyValue}>{sporsKey}</Text>
          ) : (
