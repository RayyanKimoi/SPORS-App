...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const spors_key = `${normalizedState}-${imeiPrimaryLast6}-${imeiSecondaryLast6}-${serialLast4}-${checksum}`
    ...
    const { error: updateError } = await supabase
      .from('devices')
      .update({ spors_key, ble_device_uuid })
      .eq('id', id)
    ...
    return new Response(JSON.stringify({ ok: true, spors_key, ble_device_uuid }), {
      status: 200,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    })
  } catch (error) {
